...
module JsDuck
  ...
  class HtmlGuides < GroupedAsset
    # Parses guides config file
    def initialize(opts)
      @path = File.dirname(opts.guides)
      @groups = Util::Json.read(opts.guides)
      @opts = opts
      build_map_by_name
      load_all_guides
    end
    ...
    def load_guide(guide)
      ...
      begin
        ...
        return add_toc(guide, format(Util::IO.read(guide_file)))
      rescue
    ...
    def write_guide(guide, dir)
      ...
      Util::IO.write(out_dir+"/README.html", guide[:html])
    end
